...
public:
	...
	*/
	...
	{
        if (inputString.empty() || oldString.empty())
        {
            return;
        }
        ...
        if (newString.empty())
        {
            removeInPlace(inputString,oldString,isCaseSensitive);
            return;
        }
	...
	*/
	...
	{
        if (srcString.empty() || stringToRemove.empty())
        {
            return 0;
        }
		...
		else
		{
			...
			{
			   srcString.erase(i, stringToRemove.length());
               srcStringUpper.erase(i, stringToRemoveUpper.length());
			   ++removeCount;
	...
	*/
    ...
    /**
     * @brief rightTrimmedInPlace
     * @param stringToTrim
     */
    static void rightTrimmedInPlace(std::string& stringToTrim)
    {
        int count = static_cast<int>(stringToTrim.size());
        for (int a1 = count - 1; a1 >= 0; --a1)
        {
            if (isspace(stringToTrim[a1]))
            {
                stringToTrim.erase(stringToTrim.begin() + a1);
            }
        }
    }
     ...
     * @return
     */
    static std::string rightTrimmed(const std::string& stringToTrim)
    {
        std::string outputStr = stringToTrim;
	...
	*/
     ...
     * @return
     */
    static std::vector<std::string> nullsplit(const char * str, size_t str_size)
    {
        size_t total_size = 0;
        ...
        do
        {
            ...
            ptr += values.back().size() + 1;
            total_size += values.back().size() + 1;
        }
        while(total_size < str_size);
        return values;
    }
     ...
     */
    static void multisplit(const std::string &str,
                           const std::vector<std::string>& delimiters,
                           std::vector<std::string>& output)
    {
        ...
        while (splits_available)
        {
            ...
            {
                if (!StringUtils::multicontains(item,delimiters))
                {
    ...
    * @code {.cpp}
        ...
        @endcode
    */
    static std::vector<std::string> multisplit(const std::string &str,
                                               const std::vector<std::string> &delimiters)
    {
        std::vector<std::string> output;
        multisplit(str,delimiters,output);
        return output;
    }
	...
	*/
     ...
     * @return
     */
    static bool multicontains(const std::string & value,
	...
	* @throws GenericException when items vector is empty
	* @return
	*/
						  ...
						  bool isCaseSensitive)
	{
		if (items.size() == 0)
		{
			THROW_GENERIC_EXCEPTION ("Exception is being thrown because of an empty items vector!");
		}
		...
		THROW_GENERIC_EXCEPTION(msg.c_str());
	}
	...
	*/
     ...
     * @return
     */
    static std::string multiremove(const std::string &srcString,
                                   const std::vector<std::string> &stringsToRemove,
                                   bool caseSensitve=c_DEFAULT_CASE_SENSITIVE)
    {
        std::string strOut = srcString;;
        for (const auto &stringToRemove : stringsToRemove)
        {
            strOut = StringUtils::remove(strOut,stringToRemove,caseSensitve);
        }
        return strOut;
    }
	/**
	...
	*/
	...
	{
		Random rnd;
		rnd.reseed(static_cast<unsigned int>(TimeUtils::getMillisecondsNow() + TimeUtils::getSecondsNow()));
	...
	static std::vector<size_t> getWholeWords(const std::string & source, const std::string & word)
	{
		...
		while(index != std::string::npos)
		{
			...
			if (index + word.size() < source.size() && isalnum(source[index+word.size()]))
			{
				backIsAlphaNum = true;
			}

			if (index > 0 && isalnum(source[index-1]))
			{
     ...
     * @return
     */
    static std::vector<std::string> vectorCombine(const std::vector<std::string> &vecFirst,
                                                  const std::vector<std::string> &vecSecond)
    {
        std::vector<std::string> vecNew = vecFirst;
        vecNew.insert(vecNew.end(),vecSecond.begin(),vecSecond.end());
        return vecNew;
    }

    /**
     * @brief removeAllThatDoNotEndWith
     * @param vecOut
     * @param strToRemove
     * @param caseSensitive
     * @return
											...
											bool caseSensitive = c_DEFAULT_CASE_SENSITIVE)
	{
		...
		int count = static_cast<int>(vecOut.size());
		if (count == 0 )
		{
			return 0;
		}
	...
	{
		for (int a1 = static_cast<int>(srcStr.size()) - 1; a1 >= 0; --a1)
		{
			if (!isalnum(srcStr[a1]))
			{
	 ...
	 * @return
	 */
	static std::string vectorToString(const std::vector<std::string>& values,
	 ...
	 * @return
	 */
	static std::string listToString(const std::list<std::string>& values,
								const std::string& seperator = "")
	{
		std::string out;
		size_t count = 0;
    ...
    /**
     * @brief createUniqueUUIDList
     * @param count
     * @param includeDashes
     * @return
     */
    static std::vector<std::string> createUniqueUUIDList(size_t count, bool includeDashes = false)
    {
        std::set<std::string> uuidSet;
        for (size_t a1 = 0; a1 < count; ++a1)
        {
            std::string uuid = createUUID(includeDashes);
    ...
    /**
     * @brief removeTextAfter
     * @param sourceStr
     * @param textAfterStr
     * @return
     */
    static std::string removeTextAfter(const std::string& sourceStr,
                                       const std::string& textAfterStr,
                                       bool keepTextAfter = false)
    {
        std::vector<std::string> items = StringUtils::split(sourceStr,textAfterStr);
        if (!items.empty())
        {
            return items[0] + (keepTextAfter ? textAfterStr : "");
        }
        return sourceStr;
    }
     ...
     * @return
     */
    static bool multiequals(const std::string& sourceStr,
                            ...
                            bool caseSensitive = c_DEFAULT_CASE_SENSITIVE)
    {
        for (const auto &str : comparisons)
        {
            if (caseSensitive)
            {
                if (StringUtils::trimmed(str) == StringUtils::trimmed(sourceStr))
                {
                    return true;
                }
            }
            ...
            {
                if (StringUtils::equals(sourceStr,str))
                {
                    return true;
                }
            }
